...
with tf.device('/CPU:0'):
    ...
    with tf.compat.v1.Session() as session:
        ...
        for epoch in range(trainingEpochs):
            ...
            trainingWriter = tf.compat.v1.summary.FileWriter("Logs/{}/traning".format(RunName), session.graph)
            testingWriter = tf.compat.v1.summary.FileWriter("Logs/{}/testing".format(RunName), session.graph)
        ...
        print(f"{epoch} : Final Training cost : {FinaltrainingCost} : Final TestingCost : {FinaltestingCost}")
